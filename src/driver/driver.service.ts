...
export class DriverService {
  ...
  async getDriversNearestFromLocation(latitude: number, longitude: number, driversReturnedQuantity: number): Promise<Driver[]> {
   const entityManager = this.driverRepository.manager;

    const result = await entityManager.query(
      `EXEC DriverGetNearestFromLocation
      @Latitude = ${latitude},
		  @Longitude = ${longitude},
		  @DriversReturnedQuantity = ${driversReturnedQuantity}
    `);
